...
def calculate_Eglob(G, n):
    if n <= 20:  # 小图直接算
        all_pairs = dict(nx.all_pairs_dijkstra_path_length(G, weight='weight'))
        total_efficiency = 0
        n_pairs = 0
        for i in all_pairs:
            for j, dist in all_pairs[i].items():
                if i != j and dist > 0:
                    total_efficiency += 1.0 / dist
                    n_pairs += 1
        Eglob = total_efficiency / n_pairs if n_pairs > 0 else 0
    else:  # 大图使用采样
        Eglob = calculate_global_efficiency_sampled(G, n_samples=100)
    return Eglob

def calculate_Eloc(G):
    return nx.local_efficiency(G)

# def calculate_Eglob(G):
#     return nx.global_efficiency(G)

def calcaulate_C(G):
    return nx.average_clustering(G)

def calculate_graph_metrics_fast(adj_matrix, threshold=0.1):
    """
    优化后的图论指标计算
    """
    # 确保矩阵对称
    ...
    if len(G.nodes) == 0:
        return {'C': np.nan, 'Eloc': np.nan, 'Eglob': np.nan}
...
def calculate_global_efficiency_sampled(G, n_samples=100):
    """通过采样计算全局效率"""
    nodes = list(G.nodes())
    if len(nodes) < 2:
        return 0
    ...
    for u, v in pairs:
        try:
            ...
            if path_len > 0:
                total_efficiency += 1.0 / path_len
                valid_pairs += 1
        except:
            continue
    
    return total_efficiency / valid_pairs if valid_pairs > 0 else 0
...
def process_single_sample(args):
    ...
    try:
        ...
        metrics1 = {'C': np.nan, 'Eloc': np.nan, 'Eglob': np.nan}
    
    try:
        metrics2 = calculate_graph_metrics_fast(con2_sample)
    except:
        metrics2 = {'C': np.nan, 'Eloc': np.nan, 'Eglob': np.nan}
...
def main_analysis_fast(con1, con2, label, subject_id, group_mapping=None, n_jobs=-1):
    ...
    # 收集结果
    con1_metrics = {l: {'C': [], 'Eloc': [], 'Eglob': []} for l in unique_labels}
    con2_metrics = {l: {'C': [], 'Eloc': [], 'Eglob': []} for l in unique_labels}
...
def plot_metrics_comparison(con1_metrics, con2_metrics, labels, group_names, save_path=None):
    ...
    for idx, metric in enumerate(metrics):
        ...
        #     # 使用更合理的y轴范围，留出10%的上下空间
        #     y_min = sorted_data[int(len(sorted_data) * 0.05)] if len(sorted_data) > 0 else 0
        #     y_max = sorted_data[int(len(sorted_data) * 0.95)] if len(sorted_data) > 0 else 1
        #     y_range = y_max - y_min
