...
def plot_scatter_with_fit(
    ...
    # 保存图形
    filename = f'power_channel_{channel[0]}-{channel[-1]}_{clinical_score}_r{stats_dict["pearson_r"]:.2f}_p{stats_dict["p_value"]:.3f}.png'
...
if __name__ == "__main__":
    ...
    # zsp = calculate_acg_zsp(r1, freq_bands)
    ...
    for name, time_series in zip(names, [r1, r2, r3]):
        ...
        for band_name, _ in freq_bands.items():
            # path = "./"+"output_msettest/"+band_name+f"/denoised_{name}_{band_name}_NC_vs_AD_results.csv"
            path = "./"+"output_msettest/"+band_name+f"/denoised_{name}_{band_name}_NC_vs_MCI_results.csv"
            # path = "./"+"output_msettest/"+band_name+f"/denoised_{name}_{band_name}_NC_vs_DSC_results.csv"
            ...
            if len(channels) == 0:
                print(f"Warning: No significant channels found for {name}/{band_name}")
                continue
            ...
            for channel in channels:
                for item in items:
                    result = analyze_cnn_clinical_correlation(
                        power=stats.zscore(np.log10(zsp[band_name]), axis=1),
                        channel=[channel],
