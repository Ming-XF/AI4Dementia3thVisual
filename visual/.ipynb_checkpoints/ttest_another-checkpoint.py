...
def load_and_preprocess_data(cvib=0, positive_class=0, negative_class=3):
    ...
    node_feature, adj, y, subject_id, cnn, r1, r2, r3, _ = data
...
def draw_heatmap_single(result, fig_type, cvib, pair_name, fs=22):
    """绘制单张热图并保存"""
    if fig_type == 't_stats':
        ...
        vmax = np.max(np.abs(data_matrix)) if np.any(data_matrix != 0) else 1
        vmin = -vmax
        title = 'High Effect Size (|d|>0.8)'
        cbar_label = "Cohen's d"
        fname_suffix = 'high_effect'
    ...
    im = ax.imshow(masked_data, cmap=cmap, aspect='equal',
                  vmin=vmin if vmax > 0 else 0, vmax=vmax if vmax > 0 else 1,
                  interpolation='nearest')
    ax.tick_params(axis='both', labelsize=fs)
    ax.set_xlabel('Brain Region', fontsize=fs)
    ax.set_ylabel('Brain Region', fontsize=fs)
    # ax.set_title(f'{pair_name} (cvib={cvib})\n{title}', fontsize=fs)
    ...
    filename = f'{safe_name}_cvib{cvib}_{fname_suffix}.png'
...
def create_single_images(all_results, cvib_list, pair_names, fs=22):
    """
    每个子图保存为一张独立图片，共 3(对比组) × 3(指标类型) = 9 张
    """
    fig_types = ['t_stats', 'significant', 'high_effect']
    saved_files = []

    for idx, (cvib, result, pair_name) in enumerate(zip(cvib_list, all_results, pair_names)):
        for fig_type in fig_types:
            fname = draw_heatmap_single(result, fig_type, cvib, pair_name, fs=fs)
...
def main_analysis():
    ...
    # 每个子图保存为一张独立图片，共 3×3 = 9 张，字体统一22
    saved_files = create_single_images(all_results, cvib_list, pair_names, fs=22)
    ...
    # 找出cvib=0的三个类别对共同存在的显著连接
    common_sig_df = find_common_significant_connections(results_cvib0, pair_names[:3])
