...
def draw_chord_figure(valid_connections, functional_order, new_to_orig, orig_to_new,
    ...
    for ii in sorted_indices:
        ...
        d_abs = row['abs_cohens_d']

        angle1 = angles[r1_new]
        angle2 = angles[r2_new]

        r_start = radius - ring_width
        r_stub = r_start - stub_length

        # 源端
        x1_circle = r_start * np.cos(angle1)
        y1_circle = r_start * np.sin(angle1)
        x1_stub = r_stub * np.cos(angle1)
        y1_stub = r_stub * np.sin(angle1)

        # 目标端
        x2_circle = r_start * np.cos(angle2)
        y2_circle = r_start * np.sin(angle2)
        x2_stub = r_stub * np.cos(angle2)
        y2_stub = r_stub * np.sin(angle2)

        # 控制点取两端点中点，向圆心缩进（避免所有弦都挤在圆心）
        mid_x = (x1_stub + x2_stub) / 2
        mid_y = (y1_stub + y2_stub) / 2
        cx = mid_x * 0.5
        cy = mid_y * 0.5

        t = np.linspace(0, 1, 80)
        bx = (1-t)**2 * x1_stub + 2*(1-t)*t * cx + t**2 * x2_stub
        by = (1-t)**2 * y1_stub + 2*(1-t)*t * cy + t**2 * y2_stub

        # 效应量越大线越粗
        lw = linewidths[ii]

        # 源端直桩（同ROI汇拢）
        ax.plot([x1_circle, x1_stub], [y1_circle, y1_stub],
               color=color, linewidth=lw, alpha=alphas[ii],
               solid_capstyle='round', zorder=1)
        # 过圆心的曲线
        ax.plot(bx, by, color=color, linewidth=lw, alpha=alphas[ii],
               solid_capstyle='round', zorder=1)
        # 目标端直桩
        ax.plot([x2_stub, x2_circle], [y2_stub, y2_circle],
               color=color, linewidth=lw, alpha=alphas[ii],
               solid_capstyle='round', zorder=1)
