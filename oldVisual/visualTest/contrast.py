...
def plot_results(ori_mean_corrs, vae_mean_corrs):
    ...
    # 添加统计注释
    from scipy.stats import ttest_rel
    t_stat, p_value = ttest_rel(ori_mean_corrs, vae_mean_corrs)
    ...
    # 计算均值和中位数
    ori_mean = np.mean(ori_mean_corrs)
    vae_mean = np.mean(vae_mean_corrs)
    ori_median = np.median(ori_mean_corrs)
    vae_median = np.median(vae_mean_corrs)
    
    # 添加统计信息文本框（修正了t_text的问题）
    # textstr = f'Original Data:\n  Mean: {ori_mean:.4f}\n  Median: {ori_median:.4f}\n\n' \
    #           f'Processed Data:\n  Mean: {vae_mean:.4f}\n  Median: {vae_median:.4f}\n\n' \
    #           f'Paired t-test:\n  t = {t_stat:.3f}\n  {p_text}'
    ...
    # 调整y轴范围，留出一些空间
    # y_min = min(min(ori_mean_corrs), min(vae_mean_corrs)) * 0.95
    # y_max = max(max(ori_mean_corrs), max(vae_mean_corrs)) * 1.05
