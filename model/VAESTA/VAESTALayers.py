...
class VAE(nn.Module):
    def __init__(self, view, channel1=8, channel2=16, channel3=32, d_model=32):
        ...
        self.encoder = nn.Sequential(
            nn.Conv1d(1, channel1, kernel_size=3, stride=2),
            nn.BatchNorm1d(channel1),
            nn.ReLU(),
            nn.Conv1d(channel1, channel2, kernel_size=3, stride=2),
            nn.BatchNorm1d(channel2),
            nn.ReLU(),
            nn.Conv1d(channel2, channel3, kernel_size=3, stride=2),
            nn.BatchNorm1d(channel3),
            nn.ReLU(),
            nn.Conv1d(channel3, d_model, kernel_size=3, stride=2),
        ...
        self.decoder = nn.Sequential(
            nn.ConvTranspose1d(d_model, channel3, kernel_size=3, stride=2, output_padding=1),
            nn.BatchNorm1d(channel3),
            nn.ReLU(),
            nn.ConvTranspose1d(channel3, channel2, kernel_size=3, stride=2, output_padding=1),
            nn.BatchNorm1d(channel2),
            nn.ReLU(),
            nn.ConvTranspose1d(channel2, channel1, kernel_size=3, stride=2, output_padding=1),
            nn.BatchNorm1d(channel1),
            nn.ReLU(),
            nn.ConvTranspose1d(channel1, 1, kernel_size=3, stride=2, output_padding=1),
            nn.Tanh()  # 输出在[-1,1]范围内
        )
