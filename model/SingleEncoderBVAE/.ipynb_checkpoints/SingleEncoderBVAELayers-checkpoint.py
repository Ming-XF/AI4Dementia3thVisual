...
class VAE(nn.Module):
    def __init__(self, channel1=8, channel2=16, channel3=32, d_model=32):
    ...
    def decode(self, z1, z2, z3, L):
        """解码潜在变量"""
        x1 = z1.permute(0, 2, 1)
        x1 = self.decoder1(x1)
        x1 = F.adaptive_avg_pool1d(x1, output_size=L)

        x2 = z2.permute(0, 2, 1)
        x2 = self.decoder2(x2)
        x2 = F.adaptive_avg_pool1d(x2, output_size=L)

        x3 = z3.permute(0, 2, 1)
        x3 = self.decoder3(x3)
        x3 = F.adaptive_avg_pool1d(x3, output_size=L)
        return x1, x2, x3
    ...
    def forward(self, x):
        ...
        # 解码
        # recon_x1, recon_x2, recon_x3 = self.decode(z1, z2, z3, L)
        # rl1 = nn.MSELoss(reduction='sum')(recon_x1, x)
        ...
        # fft_recon_x2 = torch.fft.fft(recon_x2, dim=-1)
        # fft_recon_x2 = torch.complex(
        #     torch.clamp(fft_recon_x2.real, min=-1e8, max=1e8),
        #     torch.clamp(fft_recon_x2.imag, min=-1e8, max=1e8)
        # )
        # f_recon_x2 = torch.abs(fft_recon_x2)
        # f_recon_x2 = torch.clamp(f_recon_x2, min=1e-8, max=1e8)
        # rl2 = nn.MSELoss(reduction='sum')(f_recon_x2, f_x)

        # p_x = torch.angle(fft_x)
        # fft_recon_x3 = torch.fft.fft(recon_x3, dim=-1)
        # fft_recon_x3 = torch.complex(
        #     torch.clamp(fft_recon_x3.real, min=-1e8, max=1e8),
        #     torch.clamp(fft_recon_x3.imag, min=-1e8, max=1e8)
        # )
        # p_recon_x3 = torch.angle(fft_recon_x3)
        # rl3 = nn.MSELoss(reduction='sum')(p_recon_x3, p_x)

        # recon_loss = rl1 + rl2 + rl3
