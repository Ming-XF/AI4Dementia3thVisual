from .SingleEncoderBVAELayers import *
from ..base import BaseConfig, ModelOutputs


class SingleEncoderBVAEConfig(BaseConfig):
    def __init__(self,
                 node_size,
                 d_model=128,
                 num_classes=2,
        ...
        super(SingleEncoderBVAEConfig, self).__init__(dropout=dropout)
...
class SingleEncoderBVAE(nn.Module):
    """
    STAGIN from https://github.com/egyptdj/stagin
    """
    def __init__(self, config: SingleEncoderBVAEConfig):
        super().__init__()
        self.config = config
        ...
        if config.abla_vae in self.vae_configs:
            ...
            # 默认情况：创建三个VAE
            self.vae = VAE(d_model=config.d_model//2)
...
#             self.linear_layers.append(nn.Linear(config.d_model, config.num_classes))
    ...
    def forward(self, time_series, node_feature, labels):
        ...
        if self.config.abla_vae in self.vae_configs:
            ...
            # mu, rl, kl = self.vae(time_series)
            mu, kl = self.vae(time_series)
            adj = self.batch_channel_pearson(mu)
            adj = torch.mean(adj, dim=1).unsqueeze(1)
