    ...
    def multiple_evaluate(self):
        ...
        with torch.no_grad():
            for inputs in evaluate_dataloader:
                ...
                con1s, con2s, ys, subject_id, cnn, r1, r2, r3, ts = outputs.logits
            ...
            r1s = np.concatenate(r1s, axis=0)
            r2s = np.concatenate(r2s, axis=0)
            r3s = np.concatenate(r3s, axis=0)
            ...
            # pdb.set_trace()
            data = [connect1s, connect2s, yss, subids, cnns, r1s, r2s, r3s, tss]
            with open('data.pkl', 'wb') as f:
                pickle.dump(data, f)
